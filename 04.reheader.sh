...
zcat /path/to/sequence.file.normID.GTflt.AB.noChrM.vcf.gz | head -n 5000 | grep "#" > /path/to/postHailHeader.txt
...
zcat /path/to/sequence.file.normID.GTflt.AB.noChrM.vcf.gz | \
  bcftools reheader -h /path/to/postHailHeader.txt | \
  bgzip > /path/to/sequence.file.normID.rehead.GTflt.AB.noChrM.vcf.gz
