...
bcftools norm -m -any --check-ref w -f /path/to/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna /path/to/sequence.file.rehead.GTflt.AB.noChrXYM.vcf.gz -Ou | \
  bcftools annotate --set-id '%CHROM:%POS:%REF:%FIRST_ALT' -Oz > /path/to/sequence.file.normID.rehead.GTflt.AB.noChrXYM.vcf.gz
